...
const i18nDict = {
  zh: {
    guide: {
      ...
      s1: '连接账号或配置 API Key',
      s1d: '前往设置页面，点击「使用 ChatGPT 登录」——无需 API Key，即可用你的 ChatGPT 订阅通过 GPT Image 生成 PPT。',
      s1i: ['也可以选择手动配置你自己的 AI 服务商和 API Key', '配置文本、图像生成模型和图像描述模型', '若需要文件解析功能，请配置 MinerU Token', '若需要可编辑导出功能，请配置MinerU TOKEN 和 Baidu API KEY'],
      s2: '保存并测试',
      ...
      feat: {
        ...
        vibe: { t: '「Vibe」式自然语言修改', d: '不再受限于复杂的菜单按钮，直接通过自然语言下达修改指令。', items: ['局部重绘：对不满意的区域进行口头式修改（如「把这个图换成饼图」）', '整页优化：基于你配置的 AI 图像模型生成高清、风格统一的页面'] },
  ...
  en: {
    guide: {
      ...
      s1: 'Connect Your Account',
      s1d: 'Open Settings and click "Sign in with ChatGPT" — no API key needed. Slides are generated with GPT Image using your ChatGPT plan.',
      s1i: ['Alternatively, configure your own AI provider and API Key in Settings', 'Configure text, image generation, and image caption models', 'If you need file parsing, configure a MinerU Token', 'If you need editable export, configure MinerU TOKEN and Baidu API KEY'],
      s2: 'Save and Test',
